...
### Function to calculate the log-likelihood of the CIR process
mllk <- function(theta.star, x, N) {
  kappa <- (theta.star[1:N])  # Mean reversion speed
  theta <- (theta.star[(N + 1):(2 * N)])  # Long-term mean level
  sigma <- (theta.star[(2 * N + 1):(3 * N)])  # Volatility coefficient
  ...
  for (j in 1:N) {
    ...
    for (i in 2:length(ind)) {  # Start from the second observation
      n_t_T <- (4 * kappa[j] * exp(-kappa[j])) / (sigma[j]^2 * (1 - exp(-kappa[j])))
      scaling_factor <- exp(-kappa[j]) / n_t_T
      df <- (4 * kappa[j] * theta[j]) / sigma[j]^2
      ncp <- n_t_T * x[ind[i - 1]]
      
      scaled_x <- (1 / scaling_factor) * x[ind[i]]
      
      allprobs[ind[i], j] <- dchisq(scaled_x, df = df, ncp = ncp) * scaling_factor
    }
  }
...
fit_HMM <- function(r, N, theta.star) {
  result <- nlm(mllk, theta.star, x = r, N = N, print.level = 0, iterlim = 10000000)
  return(c((result$estimate[1:N]), ### kappa
           (result$estimate[(N + 1):(2 * N)]), ### theta
           (result$estimate[(2 * N + 1):(3 * N)]), ### sigma
...
for (i in 1:num_simulations) {
  ...
  simulation <- simulate_HMM(n, N, kappa_true, theta_true, sigma_true, Gamma_true, delta_true, r0, C)
  r <- simulation$r
