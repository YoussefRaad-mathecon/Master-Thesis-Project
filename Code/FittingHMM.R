...
generate_theta_star <- function(N) {
  c(
    log(0.1),                            # scalar kappa
    log(seq(0.01, 0.08, length = N)),   # state-dependent theta
    log(0.2),                          # scalar sigma
    rep(-5, N * (N - 1))                # off-diagonal log-probs for Gamma
  )
}
...
generate_theta_star <- function(N) {
  c(
    log(seq(0.01, 0.2, length = N)),     # state-dependent kappa
    log(0.02),                           # scalar theta
    log(0.04),                           # scalar sigma
    rep(-5, N * (N - 1))                 # Gamma off-diagonals
  )
}
#------------------------------------2 state----------------------------------------------------#
### 2-state HMM
...
generate_theta_star <- function(N) {
  c(
    log(0.1),                            # scalar kappa
    log(0.02),                           # scalar theta
    log(seq(0.01, 0.04, length = N)),   # state-dependent sigma
    rep(-5, N * (N - 1))                # Gamma off-diagonals
  )
}
#------------------------------------2 state----------------------------------------------------#
...
generate_theta_star <- function(N) {
  c(
    log(seq(0.05, 0.2, length = N)),     # state-dependent kappa
    log(seq(0.01, 0.08, length = N)),    # state-dependent theta
    log(0.03),                           # scalar sigma
    rep(-5, N * (N - 1))                 # Gamma off-diagonals
  )
}
...
generate_theta_star <- function(N) {
  c(
    log(seq(0.5, 0.2, length = N)),     # state-dependent kappa
    log(seq(0.001, 0.08, length = N)),    # state-dependent theta
    log(seq(0.1, 0.04, length = N)),    # state-dependent sigma
    rep(-5, N * (N - 1))                 # Gamma off-diagonals
  )
}
...
N <- 5
#theta.star <- generate_theta_star(N)
theta.star <- c(log(c(0.55, 0.27, 0.01, 0.01, 0.10)),     # kappa
                log(c(0.026, 0.01228, 0.0651, 0.0247, 0.00125)),     # theta
                log(c(0.63, 0.79, 0.21, 0.16, 0.32)),     # sigma
                rep(-5, (5 - 1) * 5))
...
save(mod5, file = "mod5_252.RData")
models_list_252 <- list(
  mod2 = mod2_theta,
  mod3 = mod3_theta,
  mod4 = mod4_theta,
  mod5 = mod5_theta,
...
save(models_list_252, file = "fitted_models_252.RData")
