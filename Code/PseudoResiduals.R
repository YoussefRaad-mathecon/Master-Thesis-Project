...
# Labels for groupings
group_labels <- c(
  expression(theta), expression(kappa), expression(sigma),
  expression(theta * ", " * sigma), expression(kappa * ", " * theta),
  expression(sigma * ", " * kappa), expression(theta * ", " * sigma * ", " * kappa)
)

# Order of keys in your list (row-wise)
res_keys <- names(pseudo_residuals_list)

# Loop through each set
for (i in seq_along(res_keys)) {
  res <- pseudo_residuals_list[[res_keys[i]]]$Res
  res_finite <- res[is.finite(res)]
  
  qqnorm(res_finite, main = paste0(gsub(".*_(\\d)_.*", "\\1", res_keys[i]), "-State HMM"), 
         col = "#901a1E", xlab = "", ylab = "", cex.main = 1.5)
  qqline(res_finite, col = "steelblue", lwd = 3)
  
  missing_count <- sum(!is.finite(res))
  legend("bottomright", legend = paste0("Missing values: ", missing_count), bty = "n", cex = 0.8)
}

# Axes labels
mtext("Sample Quantiles", side = 2, line = 1.2, outer = TRUE, cex = 1.4)
mtext("Theoretical Quantiles", side = 1, line = 1.2, outer = TRUE, cex = 1.4)

# Add row labels
at_vals <- rev(((1:7) - 0.5) / 7)
for (i in seq_along(group_labels)) {
  mtext(group_labels[[i]], side = 4, line = 1, outer = TRUE, cex = 2, at = at_vals[i])
}
