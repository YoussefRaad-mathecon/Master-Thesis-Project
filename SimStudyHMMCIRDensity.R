...
N <- 5
C <- 1.5
kappa_true <- c(0.4, 0.5, 0.6, 0.7, 0.8)
theta_true <- c(0.02, 0.03, 0.04, 0.04, 0.05)
sigma_true <- c(0.04, 0.05, 0.06, 0.07, 0.08)
...
fit_HMM <- function(r, N, theta.star) {
  result <- nlm(mllk, theta.star, x = r, N = N, print.level = 0, iterlim = 1000000)
  return(c(exp(result$estimate[1:N]), ### kappa
           exp(result$estimate[(N + 1):(2 * N)]), ### theta
